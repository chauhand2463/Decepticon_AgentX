#!/usr/bin/env python3
"""
Simple test script to verify the graph_fixed.py fixes work correctly.
...
async def test_basic_functionality():
    ...
    try:
        ...
        print("[BUILD] Building graph without MCP...")
        ...
        print(f"[INPUT] Test input: {test_input}")
        print("[RUN] Running mission...")
        
        # Run the mission
        result = await graph.ainvoke(state)
        
        print("[OK] Mission completed successfully!")
        print("=" * 60)
        print("[RESULTS]:")
        ...
        if all(success_indicators):
            print("\n[SUCCESS] All tests passed! The graph fixes are working correctly.")
            return True
        else:
            print(f"\n[WARN] Some tests failed. Success indicators: {success_indicators}")
            return False
...
async def test_error_handling():
    ...
    try:
        ...
        problematic_input = "This is an invalid request with no target or clear objective"
        ...
        if result.get('phase') == 'done':
            print("[OK] Error handling test passed - graph completed despite problematic input")
            return True
        else:
            print("[ERROR] Error handling test failed - graph did not complete")
            return False
            
    except Exception as e:
        print(f"[ERROR] Error handling test failed with exception: {str(e)}")
        return False
...
async def main():
    ...
    if not os.getenv("ANTHROPIC_API_KEY"):
        ...
        print("[INFO] Testing with limited functionality (some tests may be skipped)")
    ...
    if test1_result and test2_result:
        print("\n[SUCCESS] All tests passed! The graph fixes appear to be working correctly.")
        print("[INFO] The CLI should now work better with improved error handling.")
    else:
