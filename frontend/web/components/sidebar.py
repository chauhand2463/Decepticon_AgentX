...
class SidebarComponent:
    ...
    def render_agent_status(self, container, active_agent: Optional[str] = None, completed_agents: Optional[List[str]] = None):
    ...
    def render_model_info(self, model_info: Optional[Dict[str, Any]] = None):
        if model_info:
            model_name = model_info.get('display_name', model_info.get('model', 'Unknown'))
    ...
    def render_navigation_buttons(self, callbacks: Dict[str, Callable] = None):
        ...
        if st.sidebar.button("🔁 Change Model", use_container_width=True):
            if "on_change_model" in callbacks: callbacks["on_change_model"]()
            else: st.switch_page("streamlit_app.py")

        if st.sidebar.button("📋 Chat History", use_container_width=True):
            if "on_chat_history" in callbacks: callbacks["on_chat_history"]()
            else: st.switch_page("pages/02_Chat_History.py")


        if st.sidebar.button("✨ New Chat", use_container_width=True):
            if "on_new_chat" in callbacks: callbacks["on_new_chat"]()
        ...
        if st.sidebar.button(label, use_container_width=True, key="sidebar_terminal_toggle"):
    ...
    def render_settings_section(self, callbacks: Dict[str, Callable] = None):
        ...
        debug_mode = st.sidebar.checkbox("🐞 Debug Mode", value=current_debug)
    ...
    def render_debug_info(self, debug_info: Dict[str, Any]):
        if not st.session_state.get('debug_mode'): return
        with st.sidebar.expander("🔍 Debug Info", expanded=False):
            st.json(debug_info)

    def render_complete_sidebar(self, model_info=None, active_agent=None, completed_agents=None, 
                               session_stats=None, debug_info=None, callbacks=None):
        with st.sidebar:
            ...
            if session_stats: self.render_session_stats(session_stats)
            if debug_info: self.render_debug_info(debug_info)
