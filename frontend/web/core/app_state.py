...
from src.utils.memory import create_thread_config, create_memory_namespace
...
class AppStateManager:
    ...
    def _initialize_user_session(self):
        ...
        if "thread_config" not in st.session_state:
            st.session_state.thread_config = create_thread_config(
                user_id=st.session_state.user_id, conversation_id=None
            )

        if "memory_namespace" not in st.session_state:
            st.session_state.memory_namespace = create_memory_namespace(
                user_id=st.session_state.user_id, namespace_type="memories"
            )
    ...
    def reset_session(self, keep_model: bool = True):
        ...
        if (
            hasattr(st.session_state, "logger")
            and st.session_state.logger
            and st.session_state.logger.current_session
        ):
    ...
    def create_new_conversation(self):
        ...
        st.session_state.thread_config = create_thread_config(
            user_id=st.session_state.user_id, conversation_id=new_conversation_id
        )
    ...
    def get_debug_info(self) -> Dict[str, Any]:
        ...
        debug_info = {
            ...
            "thread_id": st.session_state.get("thread_config", {})
            .get("configurable", {})
            .get("thread_id", "Not set"),
    ...
    def is_ready(self) -> bool:
        ...
        return (
            st.session_state.executor_ready
            and st.session_state.current_model is not None
            and not st.session_state.initialization_in_progress
        )
