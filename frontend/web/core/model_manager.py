...
class ModelManager:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                try:
                    ...
                    ollama_info = (
                        ...
                        if ollama_future.done()
                        else {"connected": False, "count": 0}
                    )
            ...
            if ollama_info.get("connected", False):
                result["ollama_message"] = (
                    f"Ollama Connected - {ollama_info.get('count', 0)} local models available"
                )
    ...
    def get_cached_models_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        needs_refresh = (
            force_refresh
            or not self.models_cache
            or current_time - self.cache_timestamp > self.cache_duration
        )
    ...
    def prepare_model_initialization(
        self, model_info: Dict[str, Any]
    ) -> Dict[str, Any]:

        validation_result = self.validate_model_selection(model_info)
        if not validation_result["valid"]:
            return {"ready": False, "errors": validation_result["errors"]}
        ...
        return {"ready": True, "model_info": model_info}
