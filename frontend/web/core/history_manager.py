...
class ChatHistoryManager:
    ...
    def load_sessions(self, limit: int = 20) -> Dict[str, Any]:
        ...
        if not self.logger:
            return {"success": False, "error": "Logger not available", "sessions": []}
    ...
    def _process_session_data(self, session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "start_time" in session:
            processed["formatted_time"] = self._format_session_time(
                session["start_time"]
            )
    ...
    def _apply_date_filter(
        self, sessions: List[Dict[str, Any]], date_filter: str
    ) -> List[Dict[str, Any]]:
    ...
    def _apply_sorting(
        self, sessions: List[Dict[str, Any]], sort_option: str
    ) -> List[Dict[str, Any]]:
    ...
    def validate_session_id(self, session_id: str) -> bool:
        ...
        return len(session_id) >= 32 and all(
            c.isalnum() or c == "-" for c in session_id
        )
