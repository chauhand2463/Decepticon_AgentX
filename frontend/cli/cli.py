...
from rich import markup  # noqa: E402
...
from src.utils.llm.models import list_available_models, check_ollama_connection  # noqa: E402
from src.graphs.swarm import create_dynamic_swarm  # noqa: E402
from src.swarm.graph_fixed import run_mission  # noqa: E402
from src.utils.llm.config_manager import (  # noqa: E402
    update_llm_config,
    get_current_llm_config,
    get_current_llm,
)
from src.utils.message import extract_message_content  # noqa: E402
...
from src.utils.logging.logger import get_logger  # noqa: E402
...
class DECEPTICONCLI:
    ...
    def _load_dynamic_config(self):
        try:
            ...
            self.console.print(
                f"[yellow]Warning: Could not load dynamic config: {str(e)}[/yellow]"
            )
            self.agents_config = {}
    ...
    def _load_agents_from_mcp_config(self):
        try:
            ...
            for agent_name, servers in mcp_config.items():
                ...
                actual_servers = {
                    s_name: s_config
                    for s_name, s_config in servers.items()
                    if isinstance(s_config, dict)
                    and s_name != "model"
                    and ("command" in s_config or "url" in s_config)
                }
    ...
    def display_banner(self):
        ...
        self.console.print()
        self.console.print(
            Columns([info_panel, welcome_panel], equal=True, expand=True)
        )
        self.console.print()
    ...
    async def display_mcp_infrastructure(self):
        try:
            ...
            self.tools_config = {}
            root = Tree(
                "[bold cyan]📦 MCP Agents & Tools[/bold cyan]",
            ...
            for agent_name, agent_info in self.agents_config.items():
                ...
                if not agent_info.get("servers"):
                    agent_node.add(
                        "[dim italic]⚠️  No MCP servers configured[/dim italic]"
                    )
                    continue
                ...
                for server_name, server_config in agent_info["servers"].items():
                    ...
                    server_node = agent_node.add(
                        f"[bold yellow]🖥️  Server:[/bold yellow] {server_name}"
                    )
                    ...
                    try:
                        ...
                        tools = await asyncio.wait_for(_load_tools(), timeout=10.0)
                    except asyncio.TimeoutError:
                        server_node.add(
                            "[yellow]⚠️ Server timed out (10s) — skipping[/yellow]"
                        )
                        continue
                    except Exception as e:
                        server_node.add(f"[yellow]⚠️ Error: {str(e)[:60]}[/yellow]")
                        continue

                    if tools:
                        tools_node = server_node.add(
                            "[bold magenta]🧰  Tools[/bold magenta]"
                        )
                        for tool in tools:
                        ...
                        server_node.add("[yellow]⚠️ No tools available[/yellow]")
            ...
            self.console.print(
                Panel(
                    ...
                    f"[yellow]Error:[/yellow] {markup.escape(str(e))}\n"
    ...
    def display_model_selection(self):
        ...
        with Status(
            "[bold green]Loading available models...", console=self.console
        ) as status:
            try:
                ...
                self.console.print(
                    f"[red]❌ Error loading models: {markup.escape(str(e))}[/red]"
                )
                return None
            ...
            try:
                ...
                if not selected_model["api_key_available"]:
                    if selected_model["provider"] == "ollama":
                        self.console.print(
                            "[red]❌ Ollama model not available locally.[/red]"
                        )
                    elif selected_model["provider"] == "openrouter":
                        self.console.print("[red]❌ OpenRouter API key missing.[/red]")
                    else:
                        self.console.print(
                            f"[red]❌ API key for {selected_model['provider']} missing.[/red]"
                        )
                    continue
    ...
    async def setup_session(self, model_info: Dict[str, Any]):
        with Status(
            "[bold green]Setting up session...", console=self.console
        ) as status:
            try:
                self.config = create_thread_config(
                    user_id=self.user_id, conversation_id="cli_session"
                )
    ...
    async def display_mcp_tools_info(self):
        try:
            ...
            for agent_name, agent_info in self.agents_config.items():
                ...
                for server_name, server_config in agent_info["servers"].items():
                    server_node = agent_node.add(
                        f"[bold yellow]Server:[/bold yellow] {server_name}"
                    )
                    ...
                    if self.tools_config:
                        ...
                        for tool_name, tool_info in self.tools_config.items():
                            tools_node.add(
                                f"[white]  {tool_info['display_name']}[/white]"
                            )
                    else:
        ...
        try:
            ...
            if recent_sessions:
                ...
                if len(recent_sessions) > 5:
                    self.console.print(
                        f"  [dim]... and {len(recent_sessions) - 5} more sessions[/dim]"
                    )
            else:
                self.console.print("\n[yellow]📅 No recent sessions found[/yellow]")

            self.console.print(
                f"\n[dim]📁 Logs stored at: {self.logger.base_path}[/dim]"
            )
            self.console.print("[dim]🔄 Replay-compatible logs for Streamlit[/dim]")
    ...
    async def change_model(self):
        ...
        if (
            self.current_model
            and new_model_info["model_name"] == self.current_model["model_name"]
            and new_model_info["provider"] == self.current_model["provider"]
        ):
            self.console.print(
                Panel(
        ...
        with Status(
            "[bold green]Changing model and recreating agents...", console=self.console
        ) as status:
            try:
                ...
                self.logging_session_id = self.logger.start_session(
                    new_model_info.get("display_name", "Unknown")
                )
...
• [green]model-change[/green]  - Switch between cloud or local models
...
• Talk to DECEPTICON in natural language.
• Ask for security scans, code analysis, or system reconnaissance.
• DECEPTICON uses an autonomous swarm to handle complex tasks.
...
[dim italic]Type 'quit' or 'exit' to terminate the session.[/dim italic]
    ...
    def should_display_message(self, message, agent_name, step_count):
        ...
        if msg_class in ("HumanMessage", "AIMessage", "ToolMessage"):
            if message_id not in self.processed_message_ids:
                self.processed_message_ids.add(message_id)
                return True, msg_class.replace("Message", "").lower()
            return False, None
    ...
    async def execute_workflow(self, user_input: str):
        ...
        try:
            with Status(
                "[bold cyan]DECEPTICON Swarm Active...[/bold cyan]",
                console=self.console,
                spinner="aesthetic",
                spinner_style="bold magenta",
            ) as status:
                status.update(
                    "[bold yellow]PHANTOM[/bold yellow] is architecting the mission..."
                )
                result = await run_mission(
                    user_input, self.current_llm, self.agents_config
                )

            # Build the pwned string safely — no dynamic markup inside brackets
            if result.get("exploitation_success"):
                pwned_str = "[bold red]YES[/bold red]"
            else:
                pwned_str = "[bold white]NO[/bold white]"

            self.console.print(
                Panel(
                    f"[bold green]Mission Success[/bold green]\n\n"
                    f"  STATUS    : [green]COMPLETED[/green]\n"
                    f"  PHASE     : [cyan]{markup.escape(str(result.get('phase', 'unknown'))).upper()}[/cyan]\n"
                    f"  TARGET    : [white]{markup.escape(str(result.get('target', 'unknown')))}[/white]\n\n"
                    f"  [magenta]STATISTICS[/magenta]\n"
                    f"  ├── Ports : [green]{len(result.get('open_ports', []))}[/green]\n"
                    f"  ├── CVEs  : [yellow]{len(result.get('cves', []))}[/yellow]\n"
                    f"  └── Pwned : {pwned_str}",
                    box=box.DOUBLE_EDGE,
                    border_style="bright_blue",
                    title="[bold blue] MISSION SUMMARY [/bold blue]",
                    title_align="left",
                    padding=(1, 2),
                )
            )
            ...
            if result.get("final_report"):
                self.console.print(
                    Panel(
                        ...
                        box=box.ROUNDED,
                        border_style="magenta",
                        title="[bold magenta] EXFILTRATED DATA / REPORT [/bold magenta]",
                        title_align="right",
                        padding=(1, 2),
                    )
                )
    ...
    try:
        ...
        console.print(
            Panel(
                ...
                f"[yellow]Missing dependency:[/yellow] {markup.escape(str(e))}\n\n"
        ...
        console.print(
            f"\n[bold red]❌ Startup Error: {markup.escape(str(e))}[/bold red]"
        )
...
if __name__ == "__main__":
    try:
        ...
        try:
            console.print(
                f"[bold red]❌ Critical Error: {markup.escape(str(e))}[/bold red]"
            )
        except Exception:
