...
class DECEPTICONCLI:
        ...
        try:
            ...
            for agent_name, servers in mcp_config.items():
                ...
                actual_servers = {
                    ...
                    if isinstance(s_config, dict) and s_name != "model" and ("command" in s_config or "url" in s_config)
                }
    ...
    def display_banner(self):
        ...
        self.console.print()
        self.console.print(Columns([info_panel, welcome_panel], equal=True, expand=True))
        self.console.print()
            ...
            for agent_name, agent_info in self.agents_config.items():
                ...
                for server_name, server_config in agent_info["servers"].items():
                    ...
                    try:
                        ...
                        server_node.add("[yellow]⚠️ Server timed out (10s) — skipping[/yellow]")
                        continue
                    except Exception as e:
    ...
    def display_model_selection(self):
        ...
        with Status("[bold green]Loading available models...", console=self.console) as status:
            try:
                ...
                self.console.print(f"[red]❌ Error loading models: {markup.escape(str(e))}[/red]")
                return None
            ...
            try:
                ...
                if not selected_model["api_key_available"]:
                    if selected_model["provider"] == "ollama":
                        self.console.print(f"[red]❌ Ollama model not available locally.[/red]")
                    elif selected_model["provider"] == "openrouter":
                        self.console.print(f"[red]❌ OpenRouter API key missing.[/red]")
                    else:
                        self.console.print(f"[red]❌ API key for {selected_model['provider']} missing.[/red]")
                    continue
    ...
    async def change_model(self):
        ...
        with Status("[bold green]Changing model and recreating agents...", console=self.console) as status:
            try:
                ...
                self.logging_session_id = self.logger.start_session(new_model_info.get('display_name', 'Unknown'))
    ...
    async def execute_workflow(self, user_input: str):
        ...
        try:
            with Status(
                ...
                status.update("[bold yellow]PHANTOM[/bold yellow] is architecting the mission...")
