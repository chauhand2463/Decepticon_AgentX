...
class AgentManager:
    ...
    def _load_config(cls):
        if cls._config is None:
            ...
            config_path = (
                project_root / "frontend" / "static" / "config" / "agents.json"
            )
    ...
    def normalize_agent_name(cls, agent_name: str) -> str:
        ...
        if "planner" in agent_name_lower:
            ...
            "privilege_escalation" in agent_name_lower
            or "privilege" in agent_name_lower
        ):
    ...
    def get_avatar(cls, agent_name: str) -> str:
        ...
        if normalized:
            return config["avatars"].get(
                normalized, config["avatars"].get("default", "🤖")
            )
