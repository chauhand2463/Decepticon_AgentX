...
def route_after_research(state: DecepticonState) -> str:
    ...
    cves = state.get("cves", [])
    has_critical = any(
        v.get("severity", "").upper() in ("CRITICAL", "HIGH") for v in cves
    )
...
def make_agent_node(agent, node_name: str):
    ...
    async def node_fn(state: DecepticonState) -> dict:
        ...
        try:
            if hasattr(agent, "ainvoke"):
                ...
                if not new_messages and result:
                    print(
                        f" [DEBUG] Agent {node_name} returned no messages but got result: {type(result)}"
                    )

                has_tool_calls = any(
                    hasattr(m, "tool_calls") and m.tool_calls for m in new_messages
                )
                if not has_tool_calls and node_name in ["shadow", "oracle", "breach"]:
                    print(f" [TACTICAL] {node_name.upper()} returned prose. Checking for implicit tool needs...")
                    ...
                    if fallback_data:
                        print(f" [TACTICAL] {node_name.upper()} fallback completed with new data.")
                        updates.update(fallback_data)
                    else:
                        print(f" [WARN] {node_name.upper()} fallback provided no additional data.")
            else:
                new_messages = []
        except Exception as e:
        ...
        print(f" [PERF] {node_name.upper()} execution took {elapsed:.2f}s")
        ...
        for msg in new_messages[-3:]:
            ...
            if hasattr(msg, "tool_calls") and msg.tool_calls:
                print(
                    f" [{node_name}] TOOL CALLS: {[tc['name'] for tc in msg.tool_calls]}"
                )
        ...
        if node_name == "phantom":
            ...
            print(
                f"[PHANTOM DEBUG] Last content: '{last_content[:100] if last_content else 'EMPTY'}'"
            )
        ...
        try:
            if node_name == "phantom":
                ...
                plan_data = _extract_planner_data_safe(
                    last_content, state.get("input", "")
                )
            ...
            print(
                f"[ERROR] Failed to extract structured data from {node_name}: {str(e)}"
            )
...
async def build_decepticon_graph(llm, mcp_config: dict):
    ...
    async def get_tools(agent_key: str) -> list:
        ...
        async def load_server_tools(s_name, s_config):
            try:
        ...
        # Load all servers for this agent in parallel
        server_results = await asyncio.gather(*(load_server_tools(n, c) for n, c in actual_servers.items()))
        
        all_tools = []
        for tools in server_results:
            all_tools.extend(tools)
        return all_tools

    # Parallelize tool loading for all agents
    print(" [MCP] Initializing all agents in parallel...")
    results = await asyncio.gather(
        get_tools("reconnaissance"),
        get_tools("researcher"),
        get_tools("initial_access")
    )
    
    nmap_tools, nuclei_tools, access_tools = results
...
async def build_decepticon_graph_no_mcp(llm):
    ...
    graph.add_conditional_edges(
        "shadow", route_after_recon, {"oracle": "oracle", "cipher": "cipher"}
    )
    graph.add_conditional_edges(
        "oracle", route_after_research, {"breach": "breach", "cipher": "cipher"}
    )
    graph.add_edge("breach", "cipher")
    graph.add_edge("cipher", END)
    graph.add_edge("abort", END)
...
async def build_decepticon_graph_with_tools(llm, mcp_config: dict):
    ...
    async def get_tools_simple(agent_key: str) -> list:
        ...
        async def load_server_tools_simple(s_name, s_config):
            try:
                client = MultiServerMCPClient({s_name: s_config})
        ...
        server_results = await asyncio.gather(*(load_server_tools_simple(n, c) for n, c in actual_servers.items()))
        all_tools = []
        for tools in server_results:
            all_tools.extend(tools)
        return all_tools

    # Parallelize tool loading
    results = await asyncio.gather(
        get_tools_simple("reconnaissance"),
        get_tools_simple("researcher"),
        get_tools_simple("initial_access")
    )
    recon, research, access = results
