...
def get_msf_client(host="127.0.0.1", port=55553, password="decepticon_pass", ssl=False) -> tuple:
...
def msf_error(msg: str) -> list[types.TextContent]:
    return [types.TextContent(type="text", text=json.dumps({"success": False, "error": msg}, indent=2))]
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    if name == "msf_search_module":
        ...
        try:
            ...
            # Sort by rank
            rank_order = {"excellent": 0, "great": 1, "good": 2, "normal": 3, "average": 4, "low": 5, "manual": 6}
            ...
            result = {
                ...
                "modules": [
                    ...
                    for m in filtered[:15]  # Top 15 results
                ]
            }
        except Exception as e:
    ...
    # ── MODULE INFO ──────────────────────────────────────────────────────
        ...
        try:
            ...
            result = {
                ...
                "optional_options": {k: v for k, v in options.items() if not v["required"]},
                "available_payloads": mod.targetpayloads()[:10] if hasattr(mod, "targetpayloads") else []
            }
        except Exception as e:
    ...
    # ── RUN EXPLOIT ──────────────────────────────────────────────────────
        ...
        try:
            ...
            if payload_name:
                ...
                available = exploit.targetpayloads() if hasattr(exploit, "targetpayloads") else []
                if available:
                    payload = client.modules.use("payload", available[0])
                else:
                    payload = None

            exploit_job = exploit.execute(payload=payload) if payload else exploit.execute()
            ...
            result = {
                ...
                "next_step": f"msf_session_exec with session_id={session_id}" if session_id else "Exploit did not produce a session. Try different payload or check target."
            }
        except Exception as e:
