...
def test_safe_json_parsing():
    ...
    test_cases = [
        ...
        (
            '{"target": "192.168.1.1", /* block comment */ "phase": "planning"}',
            "JS block comment",
        ),
        ...
        (
            '{"mission_type": "FULL_PENTEST", "target": "192.168.1.1",}',
            "Trailing comma",
        ),
        # Invalid JSON
...
def test_isolated_prompts():
    ...
    state = {
        ...
        "breach_results": {
            "successful_exploits": [{"service": "http", "method": "metasploit"}]
        },
    }
    ...
    for agent in agents:
        try:
            ...
            if agent == "phantom":
                ...
                if "Reconnaissance results" in str(prompt) and "open_ports" in str(
                    prompt
                ):
                    print(f"[✅] {agent}: Correct isolated prompt")
                    passed += 1
                ...
                if "Vulnerabilities found" in str(prompt) and "CVE-2021-1234" in str(
                    prompt
                ):
                    print(f"[✅] {agent}: Correct isolated prompt")
                    passed += 1
                ...
                if (
                    "Mission summary" in str(prompt)
                    and "Plan:" in str(prompt)
                    and "Recon:" in str(prompt)
                ):
                    print(f"[✅] {agent}: Correct isolated prompt")
                    passed += 1
                else:
        ...
        for i, test_input in enumerate(test_inputs, 1):
            try:
                ...
                if phase in ["done", "aborted"]:
                    print(
                        f"[{i}] Input '{test_input[:30]}...': ✅ Handled gracefully ({phase})"
                    )
                    passed += 1
                else:
                    print(
                        f"[{i}] Input '{test_input[:30]}...': ❌ Unexpected phase: {phase}"
                    )
            except Exception as e:
