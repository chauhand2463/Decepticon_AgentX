...
# FIX: make_initial_state lives in src.swarm.graph_fixed (and is re-exported from
#      src.graphs.swarm).  The original code imported it from src.swarm.graph_fixed
#      directly which works, but we centralise it through the swarm module.
...
class Executor:
    ...
    async def execute_workflow(
        ...
        execution_config: Optional[RunnableConfig] = config if config is not None else self._config

        self._processed_message_ids = set()

        # FIX: use make_initial_state so the messages list is constructed correctly
        # and the duplicate-key bug in the old dict literal doesn't apply here.
